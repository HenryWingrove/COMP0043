...
function [F0,miter] = spitzer_DB(q,K,G,d,u,b,ifht)
...
for i = 1:m
    ...
    while 1        
        ...
        F0_old = F0(i,:);
        F0(i,:) = K.*conj(G).*(K-eidxi.*Jm-eiuxi.*Jp)./L(i,:); % GFA (2.60)
        %F0(i,:) = K.*conj(G).*(K./L(i,:)-eidxi.*Pm./Lp(i,:)-eiuxi.*Qp./Lm(i,:)); % GFA (2.60)           
